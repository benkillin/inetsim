...
# http://www.thesubodh.com/2011/05/linux-iptables-best-kernel-firewall.html
# 
# iptables -A OUTPUT -p tcp --tcp-flags RST RST -j DROP
# 
# I dont like this one I want to only drop reset packets which are generated by the kernel because there is no socket bound to the specified address and port...
# "look up socket-exists in the iptables or iptables-extensions manpage --socket-exists"
#
# From the Sans Security 567 guide from the USCC cyber camp, for UDP services we'll have to block icmp port unreachable messages:
#
...
#iptables -A OUTPUT -p tcp --tcp-flags RST RST -j DROP
#iptables -A OUTPUT -p ICMP --icmp-type port-unreachable -j DROP
subprocess.Popen("iptables -A OUTPUT -p tcp --tcp-flags RST RST -j DROP", shell=True)
subprocess.Popen("iptables -A OUTPUT -p ICMP --icmp-type port-unreachable -j DROP", shell=True)
